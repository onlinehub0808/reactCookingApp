  ...
  return (
    <main className={classes.background}>
      <section className={classes.profile_section}>
        ...
        {!user.profilePic ? <img src={require(`../../assets/maleprofile.jpg`)} alt="profile"></img> :null}
        <p className={classes.profile_name}>Имена - {user.name}</p>
        <p className={classes.profile_name}>Имейл - {user.email}</p>
        {!user.profilePic ? <h3 className={classes.noPic}>Все още не си добавил профилна снимка. Покажи кой се крие за прекрасните ти рецепти и добави през "РЕДАКТИРАЙ"</h3> : null}
